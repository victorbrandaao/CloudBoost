...
* **Game-aware Session Score:** Lag Inspector now uses stricter latency and jitter thresholds for FPS/MOBA sessions.
...
* **Lag Inspector:** CloudBoost now explains likely lag-spike causes in plain language, including background traffic, jitter, packet loss, Wi-Fi/AWDL behavior, thermal pressure, and Low Power Mode.
...
The free version includes all cloud gaming, Mac gaming, and competitive game profiles, manual boost, Balanced mode, AWDL Guard rollback protection, and basic session status.

CloudBoost PRO unlocks Auto-Detect, Auto Boost, Smart Boost, Stability Guard, game-aware Lag Inspector with UDP Probe, Session Score, Heat Guard, Keep Alive, Competitive and Smooth presets, diagnostics export, and before/after session history.
